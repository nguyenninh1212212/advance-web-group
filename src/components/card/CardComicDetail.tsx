...
const CardComicDetail: React.FC<IPayload> = ({
  ...
  const handleNavigate = (e: React.MouseEvent) => {
    e.preventDefault();
    navigate(`/${encodeURIComponent(title)}/detail/${id}`);
  };

  return (
    <div
      className="w-[220px] md:h-full h-[300px] max-md:w-[200px]  px-1  relative group rounded-md"
    ...
    >
      ...
      <div className="relative overflow-hidden rounded-md md:h-[300px] h-64 ">
        ...
        >
          ...
          {/* Nút đọc ngay */}
          <button className="mt-3 bg-primary-500 px-4 py-1 text-sm rounded-lg hover:bg-primary-700 transition">
            Đọc ngay
          </button>
        </div>
      </div>
